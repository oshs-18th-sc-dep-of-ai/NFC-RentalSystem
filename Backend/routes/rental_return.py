...
# 반납 요청(React에서 `/rental_request_return/:rental_id` 호출)
@return_bp.route('/rental_request_return/<int:rental_id>', methods=['POST'])
def request_rental_return(rental_id):
    if 'session_student_id' not in session:
        return jsonify({"message": "로그인이 필요합니다.", "status": "error"}), 401

    cursor = mysql.connection.cursor()
    cursor.execute("""
        UPDATE Rentals SET rental_status = 2 WHERE rental_id = %s
    """, (rental_id,))
...
# 관리자가 반납 승인(React에서 `/admin/approve_return/:rental_id` 호출)
@return_bp.route('/admin/approve_return/<int:rental_id>', methods=['POST'])
